...
def print_tank(fish_location, fish_direction, frame, box_size):
    # Clean Tank
    tank = blank_tank()
    ...
    if fish_direction == FACE_LEFT:
        ...
        for substep in range(STEP_SIZE):
            for x in range(STEP_SIZE):
                for y in range(STEP_SIZE):
                    tank[fish_location[1]*STEP_SIZE+y][fish_location[0]*STEP_SIZE+x-substep] = fish_directed[y][x]
            ...
            # Print Tank
            draw(tank, box_size, 1/frame/STEP_SIZE)
            tank = blank_tank()
        ...
        for substep in range(STEP_SIZE):
            for x in range(STEP_SIZE):
                for y in range(STEP_SIZE):
                    tank[fish_location[1]*STEP_SIZE+y-substep][fish_location[0]*STEP_SIZE+x] = fish_directed[y][x]
            # Prepare the Tank - Add Vertical Boundary
            tankwid = getWidth(tank)
            for y in range(len(tank)):
                tank[y][0] = 1
...
def main():
    ...
    # Future Time
        ...
        # Step into Future
        print_tank(present_location, fish_direction, 90, 4)
